...
LingMXTool.generateKeil = function (vehicle_obj_list) {
    ...
    for (let i = 0; i < vehicle_obj_list.length; i++) {
        ...
        fs.writeFileSync(path.join(params.out, 'Vehicle', vehicle_obj_list[i].vehicle_name, 'vehicle.json'),
            JSON.stringify(vehicle_obj_list[i], null, 4));
    }
        ...
        .then((group_option) => {
            parseString(data, function (err, result) {
                ...
                for (let i = 0; i < vehicle_obj_list.length; i++) {
                    ...
                    let vehicle_obj = vehicle_obj_list[i];
...
LingMXTool.startCloneTasks = function (length) {
    ...
    // 检查是否已初始化
    fs.access(path.join(params.out, '.git'), fs.constants.F_OK, (err) => {
        if (err) {
            // 如果没有 .git 目录，初始化仓库
            git.init()
                .then(() => {
                    ...
                    custom_log(`Adding ${clone_task[0]} ${clone_task[2]['--branch']} to ${clone_task[1]} ${length - _clone_tasks.length}/${length}`);
                    ...
                    // });
                    // 添加子模块
                    git.submoduleAdd(clone_task[0], path.relative(params.out, clone_task[1]))
                        .then(() => {
                            git.exec(['git', 'config', `-f .gitmodules submodule.${path.relative(params.out, clone_task[1])}.branch`, clone_task[2]['--branch']])
                                .then(() => {
                                    console.log('子模块分支设置成功');
                                    LingMXTool.startCloneTasks(length);
                                })
                                .catch(err => {
                                    console.error('设置子模块分支失败', err);
                                });
                        })
                        .catch(err => {
