            ...
            parseString(data, function (err, result) {
                ...
                for (let i = 0; i < vehicle_obj_list.length; i++) {
                    ...
                    for (let j = 0; j < vehicle_obj_list[i].dep_libs.length; j++) {
                        custom_log(vehicle_obj_list[i].dep_libs[j], path.join(params.out, 'Libraries', vehicle_obj_list[i].dep_libs[j].lib_name, 'module.json'));
                        let module = fs.readFileSync(path.join(params.out, 'Libraries', vehicle_obj_list[i].dep_libs[j].lib_name, 'module.json'));
                        dep_map[vehicle_obj.dep_libs[j].lib_name] = JSON.parse(module);
                ...
                custom_log('Save to ' + path.join(params.out, 'MDK-ARM/robo_template.uvprojx'));
